...
public class ChiSquaredTest {
    ...
    public ChiSquaredResult executeTest(ArrayList<Double> randomNumbersList, int intervalsAmount, double acceptanceError) {
        ...
        for(int i = 0; i < intervalsAmount; i++) {
            intervals.add(new ChiInterval(i+1, currentMin, min + intervalWidth * (i+1), randomNumbersList.size()/intervalsAmount));
        ...
        for (ChiInterval interval : intervals) {
            System.out.println("==========================");
            System.out.println(interval.getEnd() + " - " + interval.getStart());
            while(index < randomNumbersList.size()) {
                System.out.println(randomNumbersList.get(index));
