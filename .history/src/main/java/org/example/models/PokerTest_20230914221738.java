...
public class PokerTest {
    ...
    private void fillPokerRows(int n) {
        ...
        pokerRows[0].calculateError();
        
        pokerRows[1] = new PokerRow("One Pair", onePair, 0.504, 0.504*n);
        pokerRows[1].calculateError();

        pokerRows[2] = new PokerRow("Two Pairs", twoPair, 0.108, 0.108*n);
        pokerRows[2].calculateError();

        pokerRows[3] = new PokerRow("Three Of A Kind", threeOfAKind, 0.072, 0.072*n);
        pokerRows[3].calculateError();

        pokerRows[4] = new PokerRow("Full House", fullHouse, 0.009, 0.009*n);
        pokerRows[4].calculateError();

        pokerRows[5] = new PokerRow("Poker", poker, 0.0045, 0.0045*n);
        pokerRows[5].calculateError();

        pokerRows[6] = new PokerRow("Quintet", quintet, 0.0001, 0.0001*n);
        pokerRows[6].calculateError();
    }
