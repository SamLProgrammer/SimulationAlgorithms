...
public class Congruent {
    ...
    public ArrayList<Double> fecadeStartCongruentGeneration(int x0, int k, int c, int g, int iteration, char type) {
        switch(type) {
            case 'a':
            return startCongruentGeneration(x0, 1, c, Math.pow(2,g), iteration, new ArrayList<Double>());
            case 'l':
                return startCongruentGeneration(x0,(1 + 2*k), c, Math.pow(2,g), iteration, new ArrayList<Double>());
            case 'm':
                return startCongruentGeneration(x0,(5 + 8*k), 0, Math.pow(2,g), iteration, new ArrayList<Double>());
        }
        return startCongruentGeneration(x0,(1 + 2*k), c, Math.pow(2,g), iteration, new ArrayList<Double>());
    }

    public ArrayList<Double> startCongruentGeneration(double xi, double a, double c, double m, int iteration, ArrayList<Double> risList) {
        if(iteration > 0) {
            double nextSeed = (xi * a + c) % m;
            System.out.println("Ri");
            System.out.println(nextSeed/(m));
