...
public class MeansTestPanel extends JPanel {
    ...
    private void initComponents(Controller controller) {
        ...
        JButton submitButton = new JButton("Launch Test");
    ...
    private void validateAcceptanceRate(KeyEvent e) {
        ...
        if (!((asciiCode < 48 || asciiCode > 57) && asciiCode != 46 && asciiCode != 44 && asciiCode != 8)) {
            if (acceptanceLevelField.getText().length() > 0) {
                System.out.println(myInstance().submitButton == null);
                myInstance().submitButton.setEnabled(true);
            }
        } else {
            e.consume();
            submitButton.setEnabled(false);
